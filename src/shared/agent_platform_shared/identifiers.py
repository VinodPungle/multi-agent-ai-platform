...
def new_conversation_id() -> str:
    """Return an identifier for a conversation.

    Generated by the server when a client starts a conversation without naming
    one, so a client never has to invent an id and two clients cannot collide.
    """
    return _new_id()


def new_message_id() -> str:
    """Return an identifier for a single message within a conversation.

    Assigned before generation begins, so a streaming client can create the
    element it is about to fill in and later reconcile it against the terminal
    event without matching on content.
    """
    return _new_id()
